...
Read `.agents/skills/tacos-apply-review/references/host-additional-skills.md` **Apply review applicability**. When inferred scope matches zero diff paths, return minimal tacos-format output with one INFO line (`N/A — no matching diff paths`) citing the skill path — do not run the full host rubric.
