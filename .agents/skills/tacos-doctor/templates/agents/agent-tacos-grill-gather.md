...
Read the phase bundle via `{{SKILLS_PREFIX}}/tacos-grill/references/grill-prompts.md` router table — `planning` → `grill-prompts/planning.md`; `proposal`/`specs`/`design`/`tasks` → `grill-prompts/per-phase.md` (matching `##` section); `update` → `grill-prompts/update.md`. Explore repo and schema context as needed for that bundle.
...
**Forbidden:** mode recommendations (`assumptions`, `short`, etc.), "Prefer …" labels, or grill-mode selection guidance — parent decides per `{{SKILLS_PREFIX}}/tacos-grill/references/interview-prompt.md` ## When to recommend grill mode.
